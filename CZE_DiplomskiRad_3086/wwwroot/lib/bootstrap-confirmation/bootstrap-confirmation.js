    ...
    function (_Popover) {
      ...
      _proto._copyAttributes = function _copyAttributes() {
        ...
        if (this.config.copyAttributes) {
          ...
          var ajaxCallText = $(_this2.element).attr("ajax-call");//moja linija
        var ajaxCall = ajaxCallText == "da" || ajaxCallText == "dodan" ;//moja linija

        this.config.copyAttributes.forEach(function (attr) {
          //moje rjesenje jer se funkcija 2x poziva ukoliko je dugme u tabeli; drugi poziv nije ajax
          if (attr != "href" || !ajaxCall) {
